...
public:
    ...
    void setProtocolStackSettings()
    {
        ...
        auto state = VCS_SetProtocolStackSettings(m_keyHandle, 1e6, 500, &errorCode);
    ...
    void addMotor()
    {
        ...
        auto stateType = VCS_SetMotorType(m_keyHandle, 1, 10, &errorCodeType);
        printError("VCS_SetMotorType", stateType, errorCodeType);

    
        // Function argument units (mA, mA, s/10)
        unsigned int errorCodeParameter;
        auto stateParameter = VCS_SetEcMotorParameter(m_keyHandle, 1, 6210, 10000, 310, 1, &errorCodeParameter);
    ...
    {
        unsigned int errorCodeType;
        auto sensorType = VCS_SetSensorType(m_keyHandle, 1, 1, &errorCodeType);
        printError("VCS_SetSensorType", sensorType, errorCodeType);

        unsigned int errorCodeParam;
        auto sensorParam = VCS_SetIncEncoderParameter(m_keyHandle, 1, 500, 0, &errorCodeParam);
    ...
    void setRegulatorGains()
    {
        ...
        auto statePosition = VCS_SetPositionRegulatorGain(m_keyHandle, 1, 798, 3151, 1078, &errorCodePosition);
        printError("VCS_SetPositionRegulatorGain", statePosition, errorCodePosition);

        unsigned int errorCodeVelocity;
        auto stateVelocity = VCS_SetVelocityRegulatorGain(m_keyHandle, 1, 2874, 532, &errorCodeVelocity);
        printError("VCS_SetVelocityRegulatorGain", stateVelocity, errorCodeVelocity);

        unsigned int errorCodeCurrent;
        auto stateCurrent = VCS_SetCurrentRegulatorGain(m_keyHandle, 1, 335, 60, &errorCodeCurrent);
    ...
    void activateProfilePositionMode()
    {
        ...
        auto stateSet = VCS_SetPositionProfile(m_keyHandle, 1, 1900, 9300, 9300, &errorCodeSet);
    ...
    void runProfilePositionMode()
    {
        ...
        auto moveToPos1 = VCS_MoveToPosition(m_keyHandle, 1, -24000, absoluteMovement, immediately, &errorCode);
        auto moveToPos2 = VCS_MoveToPosition(m_keyHandle, 1, 24000, absoluteMovement, immediately, &errorCode);
    ...
    void runPositionMode()
    {
        ...
        while (t < 100000)
        {
            ...
            long double position = 24000 * std::pow(std::sin(t/800*3.14), 2);
